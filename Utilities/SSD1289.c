...
#define LCD_REG      (*((volatile unsigned short *) 0x60000000))
	//0110 0000 0000 0000 0000 0000 0000 0000 A16 ->RS
...
unsigned int X_ofs=240;
unsigned int Y_ofs=10;
//****************************************************************************//
...
void LCD_WriteGRAM(void) // CHO PHÉP GHI LÊN LCD
{
  LCD_REG = LCD_REG_WR_DA_GRAM;
}
...
void LCD_Clear(uint16_t Color)
{
  ...
  LCD_WriteGRAM();
  for(index = 0; index <76800; index++)					// 76800=320*240
  {
    LCD_RAM = Color;   
  }  
}
...
{
  LCD_WriteReg(LCD_REG_GDDRAM_X,Xpos);
  LCD_WriteReg(LCD_REG_GDDRAM_Y,Ypos);
}
...
void Pixel(int16_t x, int16_t y,int16_t Color)
{ 
        if (( x > 239 )||( y > 319 )) return;
	LCD_SetCursor(x,y);
	LCD_WriteGRAM();
...
{	if(X_ofs<=asciisize)
	...
	LCD_StringLine(X_ofs,Y_ofs,str);
	X_ofs = X_ofs-asciisize;
}
...
void LCD_WriteBMP(uint8_t Xpos, uint16_t Ypos, uint8_t Height, uint16_t Width, uint8_t *bitmap)
{
  ...
  LCD_WriteReg(LCD_REG_ENTRY, 0x6048);
  ...
  LCD_WriteReg(LCD_REG_ENTRY, 0x6028);
